...
def main(_run):
    ...
    model_files = run_train(train_cfg['dataloader'], train_cfg['model'], train_cfg['scheduler'],
                            ...
                            checkpoint_frequency, restore_checkpoint_cfg, max_epochs, num_workers,
                            _run)
    if 'test' in cfg:
...
def run_train(dataloader_cfg, model_cfg, scheduler_cfg,
        ...
        restore_checkpoint, max_epochs, num_workers, _run):
    ...
    while epoch < max_epochs:
        ...
        if validation_dataloaders is not None and \
            ...
            score = evaluate(validation_dataloaders, model, epoch, device, keep=keep)
            logger.info(score)
...
def evaluate_checkpoint(_run):
    ...
    scores = evaluate_checkpoint_on(restore_checkpoint_cfg, validation_cfg, _run)
    log_score(scores, _run, "val_")
    return format_result(scores)


def evaluate_checkpoint_on(restore_checkpoint, dataset_cfg, _run):
    ...
    model = model.cuda()
    device = _run.config['device']
    return evaluate(dataloaders, model, epoch, device, keep=True)


def evaluate(dataloaders, model, epoch, device, keep=False):
    import utils
    import shutil
    scores = {}
    model = model.eval()
    ...
    for dataloader in dataloaders:
        ...
        if evaluation.name in scores:
            scores[evaluation.name].update(evaluation.score())
        else:
            scores[evaluation.name] = evaluation.score()

...
bool AddWarp::init() {
  ...
  // publisher
  success =
	  success && readPublisherParam("detected_persons_with_warp", topic, queue_size, latch);
...
bool AddWarp::readSubscriberParam(const std::string &name, std::string &topic,
  ...
  success =
	  success && nh_private_.getParam( "subscriber/" + name + "/queue_size", queue_size);
...
bool AddWarp::readPublisherParam(const std::string &name, std::string &topic,
  ...
  success =
	  success && nh_private_.getParam("publisher/" + name + "/queue_size", queue_size);
  success =
...
float AddWarp::imageDifference(const cv_bridge::CvImagePtr image_ptr,
  ...
  try {
    ...
    ROS_DEBUG_STREAM("Box size x:"
                     << detection.bbox_x << "  y " << detection.bbox_y << "  w "
                     << detection.bbox_w << "  h " << detection.bbox_h);

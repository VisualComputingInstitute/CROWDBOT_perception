...
class Trainer(object):
    ...
    def eval_epoch(self, eval_loader):
        ...
        # eval one epoch
        total_loss, total_tb_dict = [], None
        for i, data in tqdm.tqdm(enumerate(eval_loader, 0), total=len(eval_loader), leave=False, desc='eval'):
            if self._sigterm:
                break
            ...
            if total_tb_dict is None:
                total_tb_dict = {}
                for k, v in tb_dict.items():
                    total_tb_dict[k] = [v]
            else:
                for k, v in tb_dict.items():
                    total_tb_dict[k].append(v)

        # statistics this epoch
        total_loss = torch.mean(torch.tensor(total_loss)).item()
        for k, v in total_tb_dict.items():
            total_tb_dict[k] = torch.mean(torch.tensor(v)).item()

        return total_loss, total_tb_dict, disp_dict

    def train(self, num_epochs, train_loader, eval_loader=None, eval_frequency=1,
              ckpt_save_interval=5, lr_scheduler_each_iter=True):
        it = 0
            ...
            for epoch in tbar:
                ...
                for cur_it, batch in enumerate(train_loader):
                    if self._sigterm:
                        ...
                        save_checkpoint(
                                checkpoint_state(self.model, self.optimizer, epoch, it),
                    ...
                    if lr_scheduler_each_iter:
                        self.lr_scheduler.step(epoch + cur_it / len(train_loader))
                ...
                if eval_loader is not None and trained_epoch % eval_frequency == 0:
                    ...
                    with torch.set_grad_enabled(False):
                        val_loss, eval_dict, _ = self.eval_epoch(eval_loader)
                        if not self._sigterm:

...
def eval_epoch(model, test_loader, vote_kwargs, plotting=False, full_eval=True):
    model.eval()
...
def eval_epoch_with_output(model, test_loader, epoch, it, root_result_dir, split, tag,
    ...
    tb_dict, rpt_dict, fwd_dict, fig_dict = eval_epoch(
            model, test_loader, plotting=plotting, vote_kwargs=vote_kwargs,
            full_eval=full_eval)
    ...
    if writing:
        ...
        for k, v in tb_dict.items():
            ...
            if tb_log is not None:
                tb_log.add_scalar("%s_%s" % (split, k), v, it)
    ...
    if plotting:
        for k, v in fig_dict.items():
            ...
            fig, ax = v
            plt.savefig(os.path.join(fig_dir, plot_file), bbox_inches='tight')

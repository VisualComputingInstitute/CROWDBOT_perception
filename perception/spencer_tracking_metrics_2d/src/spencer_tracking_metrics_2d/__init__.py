...
class ClearMotInput(object):
    ...
    """
    def __init__(self, trackedPersonsArray, groundTruthArray,
                 matchingThreshold):
...
def calculateClearMetrics(clearMotInput):
    ...
    for cycleIdx in xrange(0, len(clearMotInput.groundTruthArray)):
        ...
        for trackedPerson in clearMotInput.trackedPersonsArray[
            ...
            if not trackedPerson.track_id in zeroBasedTrackIdMap:
                zeroBasedTrackIdMap[trackedPerson.track_id] = len(
                    zeroBasedTrackIdMap)

    # Format input data
    rospy.logdebug(
        "Formatting input data into a format readable by ClearMetrics...")
    ...
    for cycleIdx in xrange(0, len(clearMotInput.groundTruthArray)):
        ...
        for gtPerson in clearMotInput.groundTruthArray[cycleIdx].tracks:
            if not gtPerson.is_occluded:
                ...
                cmGT_thisFrame[cmId] = numpy.array([
                    gtPerson.pose.pose.position.x,
                    gtPerson.pose.pose.position.y
                ])
        ...
        # Tracker output
        cmTracks_thisFrame = [
            None for i in xrange(0, len(zeroBasedTrackIdMap))
        ]
        for trackedPerson in clearMotInput.trackedPersonsArray[
                cycleIdx].tracks:

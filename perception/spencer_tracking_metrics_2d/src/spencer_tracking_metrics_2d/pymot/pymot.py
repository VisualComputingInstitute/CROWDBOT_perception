...
class MOTEvaluation:
    ...
    def get_hypotheses_frame(self, timestamp):
        ...
        candidates = filter(hypothesis_frame_chronologically_close, self.hypotheses_["frames"])

        if len(candidates) == 0:
            print("Warning:", "No hypothesis timestamp found for timestamp %f with sync delta %f" % (timestamp, self.sync_delta_))
            return {"hypotheses": []} # empty list of hypos

        best_match = sorted(candidates, key=lambda candidate_timsestamp: abs(candidate_timsestamp['timestamp'] - timestamp))[0]

        return best_match  # return first and only element of list

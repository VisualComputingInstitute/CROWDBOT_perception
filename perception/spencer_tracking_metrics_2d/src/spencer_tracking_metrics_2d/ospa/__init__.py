...
class OspaAnalysis(ospa_implementation.OSPADistance):
    def __init__(self, p=1.0, c=10.0, a=1.0, publishResult=True):
        ...
        if self.publishResultFlag:
            self.Publisher = rospy.Publisher("/ospa_distance",
                                             ResultMsg,
                                             queue_size=1)
    ...
    def doOspaAnalysis(self, trackedPersons, groundTruth):
        ...
        timed_results = results + (float(
            trackedPersons.header.stamp.to_sec()), )
    ...
    def printAverageResults(self):
        ...
        rospy.loginfo(
            "############## Average OSPA Results #####################\nOSPA distance={}; \nLocalization Error={}; \nCardinalization Error={}; \nLabeling Error={}, \nIDswitches ={}, \nFragments={}"
            .format(averageResults[0], averageResults[1], averageResults[2],
                    averageResults[3], averageResults[4], averageResults[5]))
    ...
    def writeResultsToFile(self, filename):
        with open(filename, 'w') as f:
            # OSPA results
            f.write(
                "OSPA distance ; Localization Error ; Cardinalization Error ; Labeling Error ; IDswitches ; Fragments ;  Timestamp [s] \n"
            )

        ...
        if groundtruthPersons.header.frame_id != trackedPersons.header.frame_id:
            ...
            try:
                transformListener.waitForTransform(
                    ...
                    groundtruthPersons.header.stamp,
                    rospy.Duration(0.05),
                )
...
def transformPose(mat44, ps):
    ...
    pose44 = numpy.dot(
        tf.listener.xyz_to_mat44(ps.pose.position),
    ...
    return geometry_msgs.msg.Pose(
        geometry_msgs.msg.Point(*xyz), geometry_msgs.msg.Quaternion(*quat)
    )
...
if __name__ == "__main__":
    ...
    evaluation_base_folder = (
        expanduser("~") + "/tracking_evaluation/" + evaluationPrefix
    )
    ...
    syncQueueSize = rospy.get_param(
        "~sync_queue_size", 50
    )  # size of the synchronizer queue
    ...
    subscriberTimeout = rospy.get_param(
    ...
    synchronizedSubscribers = [
        message_filters.Subscriber(
            "/rwth_tracker/tracked_persons_2d", TrackedPersons2d
        ),
        message_filters.Subscriber("groundtruth", TrackedPersons2d),
    ]

    if approximateSync:
        timeSynchronizer = ApproximateSynchronizer(
            syncSlop, synchronizedSubscribers, syncQueueSize
        )
    else:
    ...
    totalDuration = (
        groundTruthArray[-1].header.stamp - groundTruthArray[0].header.stamp
    ).to_sec()
    rospy.loginfo(
    ...
    clearMotInput = ClearMotInput(
        trackedPersonsArray, groundTruthArray, matchingThreshold
    )
    ...
    if clearMetrics:
        ...
        writeResults(
            clearResults, evaluation_folder + "clear_metrics_%s.txt" % dateStamp
        )
    ...
    if pyMot:
        ...
        time.sleep(1)
        writeResults(
            pyMotResults, evaluation_folder + "pymot_metrics_%s.txt" % dateStamp
        )

    if ospaFlag:
        rospy.loginfo("Running OSPA analysis ...")
        for trackedPersons, groundtruthPersons in zip(
            trackedPersonsArray, groundTruthArray
        ):
            ospaAnalysis.doOspaAnalysis(trackedPersons, groundtruthPersons)
    ...
    if timingMetrics:
        ...
        with open(evaluation_folder + "timing_metrics_%s.txt" % dateStamp, "w") as f:
            ...
            for tm in timingMetricsArray:
                f.write(
                    "{};{};{};{};{};{};{};{};{};{}\n".format(
                        tm.header.stamp.to_sec(),
                        tm.cycle_no,
                        tm.track_count,
                        tm.cycle_time,
                        tm.average_cycle_time,
                        tm.cpu_load,
                        tm.average_cpu_load,
                        tm.average_processing_rate,
                        tm.elapsed_time,
                        tm.elapsed_cpu_time,
                    )
                )
    ...
    if aggregateResults:
        aggregate_results_over_runs.aggregateResults(
            evaluation_base_folder, numExpectedGTCycles
        )

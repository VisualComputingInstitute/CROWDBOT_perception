...
```
roslaunch rwth_perception_people_launch file.launch gh_queue_size:=11 vo_queue_size:=22 ubd_queue_size:=33 pt_queue_size:=44
```
...
* `model` _default = $(find rwth_ground_hog)/model/config_: The ground HOG detection models
* `gp_param_file` _default = $(find rwth_ground_plane)/config/fixed_gp.yaml_: The ground plane normal and distance
* `ptu_state` _default = /ptu/state_: The ptu state topic
* `ubd_config_file` _default = $(find rwth_upper_body_detector)/config/config_Asus.inp_: The camera config file
* `pt_config_file` _default = $(find rwth_upper_body_detector)/config/config_Asus.inp_: The camera config file
* `template_file` _default = $(find rwth_upper_body_detector)/config/upper_temp_n.txt_: The upper body templates
...
Running:
```
roslaunch rwth_perception_people_launch pedestrian_tracker_robot.launch [parameter_name:=value]
```
...
Running:
```
roslaunch rwth_perception_people_launch pedestrian_tracker_robot_with_HOG.launch [parameter_name:=value]
```
...
* `pt_queue_size` _default = 10_: The pedestrian tracking sync queue size
* `model` _default = $(find rwth_ground_hog)/model/config_: The ground HOG detection models
* `gp_config_file` _default = $(find rwth_ground_plane)/config/config_Asus.inp_: The camera config file
* `ubd_config_file` _default = $(find rwth_upper_body_detector)/config/config_Asus.inp_: The camera config file
* `pt_config_file` _default = $(find rwth_upper_body_detector)/config/config_Asus.inp_: The camera config file
* `template_file` _default = $(find rwth_upper_body_detector)/config/upper_temp_n.txt_: The upper body templates
...
Running:
```
roslaunch rwth_perception_people_launch pedestrian_tracker_standalone.launch [parameter_name:=value]
```

### pedestrian_tracker_standalone_with_HOG.launch
This depends on the rwth_ground_hog package which has to be built with the libcudaHOG. See README file of 3rd_party directory. It also uses the `/camera` namespace as a default and estimates the groundplane because it is not supposed to be run on the robot but on an external PC with a different set-up.
...
Running:
```
roslaunch rwth_perception_people_launch pedestrian_tracker_standalone_with_HOG.launch [parameter_name:=value]
```

...
Vector<double> PlaneToWorld(const Camera& camera, const Vector<double>& plane_in_camera)
{
...
stringvec readFile(const char* filename)
{
    ...
    std::cout << filename << ": Parsed " << outvec.size() << " lines." << std::endl;
...
annoframe parseTimestampString(std::string timestampString)
{
    ...
    {
        std::cout << "Malformed line in file. Could not find '.' delimiter." << std::endl;
        annoframe nope;
        return nope;
    }
              ...
              const VisualOdometry::ConstPtr &vo)
{
    ...
    {
        std::cout << std::endl;
        std::cout << "Annotation Callback: No further annotated frames available!" << std::endl;
        std::cout << "Published " << currentIndex+1 << " frames and " << publishedBoxes << " boxes." << std::endl;
    ...
    annotatedPersons.header.frame_id = "odom"; //"optitrack" for eval, "odom" for vis
    ...
    //annotatedPersons2d.header.frame_id = "rgbd_front_top_rgb_optical_frame"; //"optitrack" for eval, "odom" for vis
    annotatedPersons2d.header.frame_id = info->header.frame_id; //"optitrack" for eval, "odom" for vis
    ...
    for(auto& anno : annos)
    {
        // Convert from [x0 y0 x1 y1] to [x0 y0 w h] (Watch out: misleading name: brx is actually width, bry is height FIXME)
       double anno_width = anno.width;
       double anno_height = anno.height;
        //std::cout << "ratio w/h: " << anno.brx/anno.bry << "(id: " << anno.id << ")" << std::endl;
        //check bbox ratio and fix in a naive way (for boxes cut off at the image boundaries)

        //check if bbox is cut left/right, in case let box grow in width -> often minor differences + problems with height
        /*if(anno.tlx <1){
            //cut left
            anno.tlx -= anno_width
        ...
        if( (anno.tly+anno_height>info->height-1) && (anno_width/anno_height > 0.33) ){
        ...
        // Set pose (coordinate frame ([x y z 1]) to Spencer Tracked Persons: [z -x -y]!)
...
void connectCallback(message_filters::Subscriber<CameraInfo> &sub_cam,
    ...
    if(!pub_annotated_persons.getNumSubscribers() && !pub_annotated_persons2d.getNumSubscribers())
    {
...
int main(int argc, char **argv)
{
    ...
    private_node_handle_.param("timestamps_path", timestamps_path, string("timestamps.txt"));
    ...
    // The immediate unsubscribe is necessary to start without subscribing to any topic because message_filters does not allow to do it another way.
    ...
    ///////////////////////////////////////////////////////////////////////////////////
    // Register Callback
    sync_policies::ApproximateTime<CameraInfo, GroundPlane, VisualOdometry> MySyncPolicy(queue_size); //The real queue size for synchronisation is set here.
    ...
    ///////////////////////////////////////////////////////////////////////////////////
    // Create Publishers
    private_node_handle_.param("annotated_persons", pub_topic_annotated_persons, string("/spencer/perception/annotated_persons"));
    pub_annotated_persons = n.advertise<spencer_tracking_msgs::TrackedPersons>(pub_topic_annotated_persons, 10, con_cb, con_cb);

    private_node_handle_.param("annotated_persons2d", pub_topic_annotated_persons2d, string("/spencer/perception/annotated_persons2d"));
    pub_annotated_persons2d = n.advertise<spencer_tracking_msgs::TrackedPersons2d>(pub_topic_annotated_persons2d, 10, con_cb, con_cb);
    ///////////////////////////////////////////////////////////////////////////////////
    ...
    for(std::string timestampString : timestamps)
    {
        ...
        bool firstOccurrence = frameToAnnotatedFrame.insert(std::make_pair(frameID, annotatedFrame)).second;
        if(!firstOccurrence)
        {
            std::cerr << "Frame " << frameID << " exists multiple times!" << std::endl;
        }
    }

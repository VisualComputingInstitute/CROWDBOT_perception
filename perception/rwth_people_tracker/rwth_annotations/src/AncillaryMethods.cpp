...
void AncillaryMethods::IntersetRect(const Vector<double> &rect1,
                                    const Vector<double> &rect2,
...
void AncillaryMethods::NonMinSuppression2d(Matrix<double> &distances,
                                           Vector<Vector<double>> &pos_max,
                                           double threshold) {
  Vector<double> pos(2);
...
void AncillaryMethods::GreedyNonMaxSuppression(
    Vector<Vector<double>> &all_boxes, double overlap_threshold,
    double filter_threshold, const Matrix<double> &temp,
  ...
  while (remained_boxes.getSize() > 0) {
    ...
    for (int j = 0; j < remained_boxes.getSize(); j++) {
      ...
      AncillaryMethods::IntersetRect(remained_boxes(min_index),
                                     remained_boxes(j), rectInter);

      if (AncillaryMethods::LevelOneIOU(temp, remained_boxes(min_index),
                                        remained_boxes(j),
                                        rectInter) < overlap_threshold) {
  ...
  //    if(Globals::verbose)
  //        cout << "Reduced the Number of BBoxes from " << all_boxes.getSize()
  //        << " to " << final_bboxes_unfiltered.getSize() << endl;
  ROS_DEBUG("Reduced the Number of BBoxes from %i to %i", all_boxes.getSize(),
            final_bboxes_unfiltered.getSize());
}
...
Vector<double>
AncillaryMethods::PlaneToWorld(const Camera &camera,
...
//******************************************************************
Matrix<double> AncillaryMethods::GetDepthLibelas(int frame_nr,
                                                 const Camera &camera,
                                                 double baseline) {
...
Camera AncillaryMethods::GetCameraOrigin(const Camera &camWorld) {
  return Camera(camWorld.get_K(), Eye<double>(3), Vector<double>(3, 0.0),
                AncillaryMethods::PlaneToCam(camWorld));
}

void AncillaryMethods::MinMaxVecVecDim(Vector<Vector<double>> &v,
                                       int second_dim, double &min,
                                       double &max) {
  min = v(0)(second_dim);
  max = v(0)(second_dim);
  double current;
...
void AncillaryMethods::MinMaxIndexVecVecDim(Vector<Vector<double>> &v,
                                            int second_dim, int &index_min,
                                            int &index_max) {
...
double AncillaryMethods::MinVecVecDim(Vector<Vector<double>> &v, int second_dim,
                                      int &index) {
...
double AncillaryMethods::MaxVecVecDim(Vector<Vector<double>> &v, int second_dim,
                                      int &index) {
...
double AncillaryMethods::MedianOfMatrixRejectZero(const Matrix<double> &mat,
...
double AncillaryMethods::LevelOneIOU(const Matrix<double> &temp,
                                     const Vector<double> &bbox1,
                                     const Vector<double> &bbox2,
                                     const Vector<double> &boxIntersect) {
...
double AncillaryMethods::DistanceToPlane(const Vector<double> &point,
                                         const Vector<double> &gp) {
  return gp(0) * point(0) + gp(1) * point(1) + gp(2) * point(2) +
         gp(3) * Globals::WORLD_SCALE;
}

// void AncillaryMethods::RenderBBox2DWithScore(const Vector<double>& bbox,
// QImage& image, int r, int g, int b, int lineWidth)
//{
...
Vector<double> AncillaryMethods::getGaussian1D(double sigma, double precision) {
  ...
  for (int i = 0; i < size; i++) {
    x = i - center;
    kernel(i) =
        exp(x * x / (-2.0 * sigma * sigma)) / (sigma * sqrt(2.0 * M_PI));
    sum += kernel(i);
  }
...
Vector<double> AncillaryMethods::conv1D(const Vector<double> &vec,
...
void AncillaryMethods::MorphologyErode(Matrix<double> &img) {
  ...
  for (int x = 1; x <= x_size; ++x) {
    for (int y = 1; y <= y_size; ++y) {
      if (img_extend(x, y) > 0) {
        if (!(img_extend(x - 1, y) > 0 && img_extend(x, y - 1) > 0 &&
              img_extend(x + 1, y) > 0 && img_extend(x, y + 1) > 0))
          img(x - 1, y - 1) = 0;
      }
    }
  }
...
void intersectPlane(Vector<double> &gp, double gpd, Vector<double> &ray1,
                    Vector<double> &ray2, Vector<double> &point) {
...
void getRay(const Vector<double> &x, Vector<double> &ray1, Vector<double> &ray2,
            const Camera &cam) {
...
Vector<double> AncillaryMethods::backprojectGP(const Vector<double> &pos2D,
                                               const Camera &cam,
                                               const Vector<double> &gp) {
  Vector<double> gpN(3);
  gpN(0) = (gp(0));
...
void AncillaryMethods::intersectIdx(Vector<FrameInlier> &idx1,
  ...
  if (idx1.getSize() > 0 && idx2.getSize() > 0) {
    for (int i = 0; i < idx1.getSize(); i++) {
      ...
      while (idx1(i).getFrame() > idx2(j).getFrame() &&
             j < idx2.getSize() - 1) {
        j++;
      }
...
Matrix<double> AncillaryMethods::smoothTrajMatrix(Matrix<double> &allPoints,
                                                  int nSmoothSize) {
...
void AncillaryMethods::compute_rectangle(Vector<double> &main4D,
                                         Vector<double> &ort4D,
                                         Vector<double> &Lmax,
                                         Vector<double> &x,
...
Matrix<double> AncillaryMethods::conv1D(Matrix<double> &im,
                                        Vector<double> &kernel, bool dirFlag) {
...
void AncillaryMethods::ExtractSlopsOnBorder(const Matrix<double> &image,
                                            Vector<double> &ys,
                                            Vector<double> &slopes, int start_x,
...
void AncillaryMethods::ExtractBorder(const Matrix<double> &image,
                                     Vector<double> &ys, int start_x, int end_x,
...
Vector<double> AncillaryMethods::fromWorldToCamera(Vector<double> &posInWorld,
                                                   Camera &cam) {
...
Vector<double> AncillaryMethods::fromCameraToWorld(Vector<double> &posInCamera,
                                                   Camera &cam) {

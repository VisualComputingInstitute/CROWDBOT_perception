...
Camera::Camera(const Matrix<double> &K, const Matrix<double> &R,
               const Vector<double> &t, const Vector<double> &GP) {
  this->K_ = K;
  this->R_ = R;
  this->t_ = t;
  this->GP_ = GP;
...
void Camera::WorldToImage(const Vector<double> &pos3D, double world_scale,
                          Vector<double> &pos2D) const {
  ...
  if (vKrt(2) == 0) {
    //        if(Globals::verbose)
    //            cout <<"'Warning in WorldToImage(): transformed point is
    //            invalid!'" << endl;
...
void Camera::ProjectToGP(const Vector<double> &pos3D, double world_scale,
                         Vector<double> &pos3D_on_plane) const {
  double d = DotProduct(GPN_, pos3D) + GPD_ * world_scale;
  pos3D_on_plane = pos3D - GPN_ * d;
}

// Modified to R and t instead of Rt and Rt*t
void Camera::getRay(Vector<double> &x, Vector<double> &ray1,
                    Vector<double> &ray2) {
...
double Camera::bbToDetection(const Vector<double> &bbox, Vector<double> &pos3D,
                             double ConvertScale, double &dist) {
  //    pos3D.clearContent();
  pos3D.setSize(3);
  ...
  // printf("GPD\n");
  // printf("%f\n",GPD_);
  // printf("ray_bot_left_1\n");
  // ray_bot_left_1.show();
  // printf("ray_bot_left_2\n");
  // ray_bot_left_2.show();
  ...
  aux(0) = x + w / 2.0; // FIXED: move top point in middle, s.t. height is
                        // computed correctly lateron
  aux(1) = y;
...
void Camera::intersectPlane(Vector<double> &gp, double gpd,
                            Vector<double> &ray1, Vector<double> &ray2,
                            Vector<double> &point) {
...
void Camera::jacFor3DCov(Vector<double> &X, Matrix<double> &Cov) {
  ...
  double denom = (R(2, 0) * (X(0) - t(0)) + R(2, 1) * (X(1) - t(1)) +
                  R(2, 2) * (X(2) - t(2))) *
                 (R(2, 0) * (X(0) - t(0)) + R(2, 1) * (X(1) - t(1)) +
                  R(2, 2) * (X(2) - t(2)));

  double num1 =
      K(0, 0) * ((X(1) - t(1)) * (R(0, 0) * R(2, 1) - R(2, 0) * R(0, 1)) +
                 (X(2) - t(2)) * (R(0, 0) * R(2, 2) - R(0, 2) * R(2, 0)));

  double num2 =
      K(0, 0) * ((X(0) - t(0)) * (R(0, 1) * R(2, 0) - R(0, 0) * R(2, 1)) +
                 (X(2) - t(2)) * (R(0, 1) * R(2, 2) - R(2, 1) * R(0, 2)));

  double num3 =
      K(0, 0) * ((X(0) - t(0)) * (R(0, 2) * R(2, 0) - R(0, 0) * R(2, 2)) +
                 (X(1) - t(1)) * (R(0, 2) * R(2, 1) - R(2, 2) * R(0, 1)));

  double num4 =
      K(1, 1) * ((X(1) - t(1)) * (R(1, 0) * R(2, 1) - R(2, 0) * R(1, 1)) +
                 (X(2) - t(2)) * (R(1, 0) * R(2, 2) - R(2, 0) * R(1, 2)));

  double num5 =
      K(1, 1) * ((X(0) - t(0)) * (R(1, 1) * R(2, 0) - R(2, 1) * R(1, 0)) +
                 (X(2) - t(2)) * (R(1, 1) * R(2, 2) - R(2, 1) * R(1, 2)));

  double num6 =
      K(1, 1) * ((X(0) - t(0)) * (R(1, 2) * R(2, 0) - R(2, 2) * R(1, 0)) +

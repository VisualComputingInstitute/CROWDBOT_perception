...
Camera createCamera(Vector<double> &GP, const VisualOdometry::ConstPtr &vo,
  ...
  Vector<double> GP_world = AncillaryMethods::PlaneToWorld(
      camera, GP); // make sure AncillaryMethods are imported
...
void callback(const CameraInfoConstPtr &info, const GroundPlane::ConstPtr &gp,
  ...
  for (auto &anno : annos) {
    ...
    Vector<double> bbox = {anno.tlx, anno.tly, anno.brx - anno.tlx,
                           anno.bry - anno.tly};
    ...
    // posInWorld then in our coordinate frame ([x y z 1]), for Spencer Tracked
    // Persons: [z -x -y]!! create TrackedPersons-Message + publish
  }

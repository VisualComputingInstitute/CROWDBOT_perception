...
public:
  ...
  void appendToTXT(char *filename);
...
template <class T>
inline Vector<T> operator*(const Vector<T> &aVector, const Vector<T> &bVector);
// Dot product
...
template <class T>
Vector<T> hist(Vector<T> &vec, int nrBins, bool normalize, T st, T en);
...
template <class T> Vector<T>::Vector(int sizeO) { dataC.resize(sizeO); }
...
// appending constructor
template <class T>
Vector<T>::Vector(const Vector<T> &vec1, const Vector<T> &vec2) {
...
// copy from STL vector constructor0
template <class T> Vector<T>::Vector(const vector<T> &vecO) { dataC = vecO; }
...
template <class T>
Vector<T>::Vector(const T &x, const T &y, const T &z, const T &a) : dataC(4) {
  //    dataC.resize(4);
  dataC[0] = x;
  dataC[1] = y;
  dataC[2] = z;
...
template <class T>
Vector<T>::Vector(const T &x, const T &y, const T &z) : dataC(3) {
  //    dataC.resize(3);
  dataC[0] = x;
  dataC[1] = y;
  dataC[2] = z;
...
template <class T> int Vector<T>::getSize() const { return dataC.size(); }
...
template <class T>
Vector<T> operator*(const Vector<T> &aVector, const Vector<T> &bVector) {
...
template <class T> void Vector<T>::setSize(int sizeO) { dataC.resize(sizeO); }
...
template <class T> void Vector<T>::clearContent() { dataC.clear(); }
...
template <class T>
void Vector<T>::intersection(Vector<T> &vec2, Vector<T> &intersect) {
...
template <class T> void Vector<T>::sortV() { sort(dataC.begin(), dataC.end()); }
...
Vector<T> hist(Vector<T> &vec, int nrBins, bool normalize, T st, T en) {
  ...
  for (T i = st; i < en; i += step) {
    ...
    for (int j = 0; j < vec.getSize(); j++) {
      ...
      // Make sure if a element in the vector equals en we need to add to the
      // last bin.
      if (vec(j) == en) {
...
template <class T> T *Vector<T>::data() { return &dataC[0]; }
...
template <class T> void Vector<T>::appendToTXT(char *filename) {
  ofstream aStream;

...
public:
  ...
  // Create a matrix and fill it with a vector of vector (convert vector of
  // vector to matrix)
  Matrix(Vector<Vector<T>> &vector_of_vector);
  // Create a matrix and fill it with a specific part of another matrix
  Matrix(const Matrix<T> &source, int start_column, int end_column,
  ...
  void appendToTXT(const char *filename, const int flag_size = 0) const;
...
// matrix difference
template <class T>
Matrix<T> operator-(Matrix<T> const &aM1, Matrix<T> const &aM2);
// matrix product
template <class T>
Matrix<T> operator*(Matrix<T> const &aM1, Matrix<T> const &aM2);
// Multiplication with a vector
template <class T>
Vector<T> operator*(Matrix<T> const &aMatrix, Vector<T> const &aVector);
...
template <class T>
T maxOfMatrix(const Matrix<T> &aMatrix, int &xPos, int &yPos);
...
template <class T>
Matrix<T>::Matrix(const int x_size, const int y_size, const T &fill_value) {
  x_size_ = x_size;
  y_size_ = y_size;
...
// filling constructor
template <class T>
Matrix<T>::Matrix(const int xSizeO, const int ySizeO, const T *data) {
  x_size_ = xSizeO;
  y_size_ = ySizeO;
...
template <class T>
Matrix<T>::Matrix(const Matrix<T> &source, int start_column, int end_column,
                  int start_row, int end_row) {
  assert(start_column <= end_column && start_row <= end_row &&
         start_column >= 0 && end_column >= 0 && start_row >= 0 &&
         end_row >= 0 && end_column < source.x_size() &&
         end_row < source.y_size());
...
// Operator * vector
template <class T>
inline Matrix<T> &Matrix<T>::operator*=(const Vector<T> &vec) {
...
template <class T> inline int Matrix<T>::y_size() const { return y_size_; }

template <class T> inline int Matrix<T>::x_size() const { return x_size_; }

template <class T> inline int Matrix<T>::total_size() const {
  return x_size_ * y_size_;
}

template <class T> T *Matrix<T>::data() const { return data_; }
...
template <class T>
void Matrix<T>::set_size(int x_size, int y_size, const T &fill_value) {
  if (data_ != 0)
    delete[] data_;
  data_ = new T[x_size * y_size];
  x_size_ = x_size;
...
template <class T>
void Matrix<T>::insert(Matrix<T> &mat, int start_x, int start_y) {
  assert(x_size_ >= mat.x_size() + start_x &&
         y_size_ >= mat.y_size() + start_y);
...
template <class T>
void Matrix<T>::ReadFromTXT(string filename, int x_size, int y_size) {
  const char *p;
  p = filename.c_str();
  ifstream aStream(p);
...
template <class T>
void Matrix<T>::WriteToTXT(const char *filename, const int flag_size) const {
...
template <class T>
void Matrix<T>::appendToTXT(const char *filename, const int flag_size) const {
  ofstream aStream;
  aStream.open(filename, std::ios_base::app);
  if (flag_size == 1) {
    aStream << y_size_ << " " << x_size_ << endl;
  } else if (flag_size == 2) {
    aStream << y_size_ << " " << endl;
...
template <class T>
Vector<T> operator*(Matrix<T> const &mat, Vector<T> const &vec) {
...
template <class T>
void Matrix<T>::getColumn(const int colNumber, Vector<T> &target) {
...
template <class T>
void Matrix<T>::getRow(const int rowNumber, Vector<T> &target) {
...
template <class T>
void Matrix<T>::cutPart(int startC, int endC, int startR, int endR,
                        Matrix<T> &target) {
  assert(startC <= endC && startR <= endR && startC >= 0 && endC >= 0 &&
         startR >= 0 && endR >= 0 && endC < x_size_ && endR < y_size_);
...
template <class T>
void Matrix<T>::insertRow(Vector<T> &src, const int rowNumber) {
...
template <class T>
void Matrix<T>::transferVecVecToMat(Vector<Vector<T>> &objO) {

                                ...
                                map<int, int, greater<int> >& hypoLastSelForVis, int t, int hypoID)
{
    ...
    if (!colorFlag)
    {
        colorPos = int (rand() % colors.y_size());
                             ...
                             double r, const unsigned char* color, double height)
{
    ...
    for(double t = 0.1; t<=2*M_PI; t+=0.1)
    {
        ...
        image.draw_line_1(lp(0),lp(1),p(0),p(1),color,3);
        image.draw_line_1(lp2(0),lp2(1),p2(0),p2(1),color,3);
...
//    Vector<double> tc(bc(0),bc(1)-h,bc(2));
    cam.WorldToImage(bc,Globals::WORLD_SCALE,p1);
    cam.WorldToImage(tc,Globals::WORLD_SCALE,p2);
...
//    p2=render_circle(cam,image,tc,r,color);
    image.draw_line_1(p1(0),p1(1),p1(4),p1(5),color,3);
    image.draw_line_1(p1(2),p1(3),p1(6),p1(7),color,3);

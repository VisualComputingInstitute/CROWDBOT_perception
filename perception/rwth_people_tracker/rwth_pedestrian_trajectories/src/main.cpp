...
public:
  ...
  ~HelperSearchInterface() { ofs.close(); }
  ...
  void clearBlacklist() { id_blacklist_.clear(); }
  ...
  bool use_map = true; // True to take into account of obstacle when selecting
                       // helpers, using an existing map.
  bool strict = false; // True if a selected ID needs to fulfill the criteria
                       // all the time.
  // bool remember = true;  // True then a once selected ID will always be
  // considered as potential helper, if it fulfills the criteria (or strict is
  // false)
  bool keep =
      true; // True will give priority to last selected person for helper.
...
void callback_newSearch(const std_msgs::Bool::ConstPtr &msg) {
  hs_.ofs << ros::Time::now() << ": enter new_search callback with data="
          << (msg->data ? "True" : "False") << "\n";
  hs_.active = true;
  ...
  hs_.ofs << ros::Time::now() << ": exit new_search callback"
          << "\n";
}

void callback_resetHelperBlacklist(const std_msgs::Bool::ConstPtr &msg) {
  hs_.ofs << ros::Time::now() << ": enter resetBlacklist callback with data="
          << (msg->data ? "True" : "False") << "\n";
  hs_.clearBlacklist();
  hs_.ofs << ros::Time::now() << ": exit resetBlacklist callback"
          << "\n";
}
...
void callback_stopHelperSelection(const std_msgs::Bool::ConstPtr &msg) {
  ...
  hs_.ofs << ros::Time::now() << ": exit stopHelperSelection callback"
          << "\n";
...
bool checkReachability(const geometry_msgs::PointStamped &start_pt,
  ...
  try {
    tf_->waitForTransform(start_pt.header.frame_id, map_frame,
                          start_pt.header.stamp, ros::Duration(1.0));
    tf_->transformPoint(map_frame, start_pt, start_pt_map);
    tf_->transformPoint(map_frame, goal_pt, goal_pt_map);
  } catch (tf::TransformException ex) {
    ROS_WARN_THROTTLE(
        20.0,
        "Failed transform lookup in rwth_pedestrian_trajectories. Reason: %s. "
        "Message will re-appear within 20 seconds. Ignore map check.",
        ex.what());
    return true;
  }
  ...
  // std::cout << "Start pose (x, y): " << start_stamp_map.point.x << " " <<
  // start_stamp_map.point.y << " "; std::cout << "Goal pose (x, y): " <<
  // goal_stamp_map.point.x << " " << goal_stamp_map.point.y << " "; std::cout
  // << "Map pose (x, y): " << map_x << " " << map_y << std::endl;
...
int updateTrajectories(const TrackedPerson &tp) {
  ...
  if (idx >= 0) {
    ...
    if (matching_trj.size() > trajectory_max_length_) {
      std::rotate(matching_trj.begin(), matching_trj.begin() + 1,
                  matching_trj.end());
...
bool getRobotLocation(geometry_msgs::PointStamped &pt_stamped_target) {
  ...
  try {
    tf_->waitForTransform(target_frame, camera_frame_, time,
                          ros::Duration(1.0));
    ...
    ROS_WARN_THROTTLE(
        20.0,
        "Failed transform lookup in rwth_pedestrian_trajectories. Reason: %s. "
        "Message will re-appear within 20 seconds.",
        ex.what());
    return false;
  }
...
bool isGoodHelper(const PersonTrajectory &pt,
  ...
  // Distance
  const geometry_msgs::Point &person_loc_point =
      pt.trajectory.back().pose.pose.position;
...
int selectHelper(const std::vector<int> &trajectory_indices,
  ...
  const std::vector<PersonTrajectory> &trajectories =
      person_trajectories_.trajectories;
  helper_flag.clear();
  ...
  // Loop through all tracked trajectories, and check if the person is a good
  // helper.
  int selected_idx = -1;
...
void callback(const TrackedPersons::ConstPtr &tps) {
  ...
  s_viz.pose.pose.position =
      selected_person_trajectory.trajectory.back().pose.pose.position;
...
int main(int argc, char **argv) {
  ...
  private_node_handle_.param("tracked_persons", sub_topic_tracked_persons,
                             std::string("/rwth_tracker/tracked_persons"));
  private_node_handle_.param("get_new_helper", sub_topic_new_search,
                             std::string("/rwth_tracker/get_new_helper"));
  private_node_handle_.param(
      "reset_helper_blacklist", sub_topic_reset_helper_blacklist,
      std::string("/rwth_tracker/reset_helper_blacklist"));
  private_node_handle_.param(
      "stop_helper_selection", sub_topic_stop_helper_selection,
      std::string("/rwth_tracker/stop_helper_selection"));
  private_node_handle_.param("camera_frame", camera_frame_,
                             std::string("/camera/"));
  ...
  // private_node_handle_.param("helper_reid_thresh", helper_reid_thresh,
  // double(50));
  ...
  message_filters::Subscriber<std_msgs::Bool> subscriber_new_search(
      n, sub_topic_new_search.c_str(), 1);
  subscriber_new_search.unsubscribe();
  subscriber_new_search.registerCallback(boost::bind(&callback_newSearch, _1));
  subscriber_new_search.subscribe();
  message_filters::Subscriber<std_msgs::Bool> subscriber_reset_helper_blacklist(
      n, sub_topic_reset_helper_blacklist.c_str(), 1);
  subscriber_reset_helper_blacklist.unsubscribe();
  subscriber_reset_helper_blacklist.registerCallback(
      boost::bind(&callback_resetHelperBlacklist, _1));
  subscriber_reset_helper_blacklist.subscribe();
  message_filters::Subscriber<std_msgs::Bool> subscriber_stop_helper_selection(
      n, sub_topic_stop_helper_selection.c_str(), 1);
  subscriber_stop_helper_selection.unsubscribe();
  subscriber_stop_helper_selection.registerCallback(
      boost::bind(&callback_stopHelperSelection, _1));
  subscriber_stop_helper_selection.subscribe();

  // Listen to map
  std::string sub_topic_map;
  private_node_handle_.param("maps", sub_topic_map,
                             std::string("/map_latched"));
  ...
  private_node_handle_.param("person_trajectories", pub_topic_trajectories,
                             std::string("/rwth_tracker/person_trajectories"));
  private_node_handle_.param("selected_helper", pub_topic_selected_helper,
                             std::string("/rwth_tracker/selected_helper"));
  private_node_handle_.param("potential_helpers", pub_topic_potential_helpers,
                             std::string("/rwth_tracker/potential_helpers"));
  private_node_handle_.param("selected_helper_vis",
                             pub_topic_selected_helper_vis,
                             std::string("/rwth_tracker/selected_helper_vis"));
  private_node_handle_.param(
      "potential_helpers_vis", pub_topic_potential_helpers_vis,
      std::string("/rwth_tracker/potential_helpers_vis"));
  private_node_handle_.param("helper_search_status",
                             pub_topic_helper_search_status,
                             std::string("/rwth_tracker/helper_search_status"));
  private_node_handle_.param("helper_selected", pub_topic_helper_selected,
                             std::string("/rwth_tracker/helper_selected"));
  person_trajectories_pub_ = n.advertise<PersonTrajectories>(
      pub_topic_trajectories, 1, con_cb, con_cb);
  selected_helper_pub_ = n.advertise<PersonTrajectory>(
      pub_topic_selected_helper, 1, con_cb, con_cb);
  good_helpers_pub_ = n.advertise<PersonTrajectories>(
      pub_topic_potential_helpers, 1, con_cb, con_cb);
  selected_helper_viz_pub_ = n.advertise<DetectedPersons>(
      pub_topic_selected_helper_vis, 1, con_cb, con_cb);
  good_helpers_viz_pub_ = n.advertise<DetectedPersons>(
      pub_topic_potential_helpers_vis, 1, con_cb, con_cb);

  hs_.status_pub = n.advertise<std_msgs::Bool>(pub_topic_helper_search_status,
                                               1, con_cb, con_cb);
  hs_.stop_ack_pub = n.advertise<std_msgs::Bool>(
      sub_topic_stop_helper_selection + "_ACK", 1, con_cb, con_cb);
  hs_.start_ack_pub = n.advertise<std_msgs::Bool>(sub_topic_new_search + "_ACK",
                                                  1, con_cb, con_cb);
  hs_.success_pub =
      n.advertise<std_msgs::Bool>(pub_topic_helper_selected, 1, con_cb, con_cb);

...
class OSPADistance():
    def __init__(self, p=2.0, c=10.0, a=5.0):
        ...
        if self.use_lapjv:
            try:
                ...
                rospy.logerr(
                    "Setting use_lapjv in OSPA to False because required library could not be found. Install from http://www.starklab.org/members/kazmar/software/!"
                )
        ...
        for i in X:
            for j in Y:
                distance = numpy.linalg.norm(
                    numpy.asarray(i['position']) -
                    numpy.asarray(j['position']))
                if distance <= self.c:
    ...
    def evaluateFrame(self, X_Tracks, Y_GT):
        ...
        for gt_id in self.gt_mappings.keys():
            ...
            distance = numpy.linalg.norm(
                numpy.asarray(groundtruth[0]['position']) -
                numpy.asarray(hypothesis[0]['position']))
            if distance <= self.c:
                rospy.logdebug("Keeping correspondence between %s and %s" %
                               (gt_id, self.gt_mappings[gt_id]))
        ...
        if len(existing_X_ids) > 0:
            X_Tracks = filter(lambda h: h['id'] not in existing_X_ids,
                              X_Tracks)
        ...
        if len(X_Tracks) > 0 or len(Y_GT) > 0:
            ...
            if munkres_matrix.min() == self.c:
                indices = zip(range(max(len(X_Tracks), len(Y_GT))),
                              range(max(len(X_Tracks), len(Y_GT))))
            elif len(munkres_matrix) > 1:
                if self.use_lapjv:
                    lap_result = self.lapjv.lap(
                        numpy.array(munkres_matrix, dtype=numpy.float32))
                    indices = zip(range(max(len(X_Tracks), len(Y_GT))),
                                  lap_result[1])
                else:
                    munkres = Munkres()
            ...
            for [t, g] in indices:
                ...
                if dists[t][g] < self.c:
                    if Y_GT[g]['id'] in self.gt_mappings.keys(
                    ) and self.gt_mappings[Y_GT[g]['id']] != X_Tracks[t]['id']:
                        rospy.logdebug(
                            "Detected id switch for ground truth track {} assigned before to {} now assigned to {}"
                            .format(Y_GT[g]['id'],
                                    self.gt_mappings[Y_GT[g]['id']],
                                    X_Tracks[t]['id']))
                        fragments += 1
                    ...
                    rospy.logdebug(
                        "Found new mapping for gt_id {} to track_id {}".format(
                            Y_GT[g]['id'], X_Tracks[t]['id']))
        ...
        ospa_tuple = (float(ospa_err), float(err_loc), float(err_cn),
                      float(err_label), float(wrong_labels), float(fragments))
